...
function xt_preview_theme()
{
    ...
    $allowed = [$xoopsConfig['theme_set'], $dir];
...
function xt_activate_theme()
{
    ...
    $sql = 'UPDATE ' . $db->prefix('config') . " SET conf_value='" . serialize([$dir]) . "' WHERE conf_modid=0 AND conf_catid=1 AND conf_name='theme_set_allowed'";
    $db->queryF($sql);
...
function xt_install_theme()
{
    ...
    if (is_file($theme_dir . '/assemble/' . $dir . '.theme.php')) {
        ...
        require_once $theme_dir . '/assemble/' . $dir . '.theme.php';
        $class = ucfirst($dir);
...
function xt_uninstall_theme()
{
    ...
    if (!$theme || 'standard' == $theme->getInfo('type')) {
